// Emoji shortcut support for GitHub-style markdown

export interface EmojiMap {
  [shortcode: string]: string;
}

// Common GitHub emoji shortcuts
export const GITHUB_EMOJIS: EmojiMap = {
  // Smileys & Emotion
  ":smile:": "😄",
  ":laughing:": "😆",
  ":blush:": "😊",
...
export function replaceEmojiShortcodes(
  text: string,
  emojiMap: EmojiMap = GITHUB_EMOJIS,
): string {
  let result = text;
...
export function detectEmojiShortcodes(
  text: string,
  emojiMap: EmojiMap = GITHUB_EMOJIS,
): string[] {
...
export function getEmojiSuggestions(
  ...
  const suggestions: Array<{ shortcode: string; emoji: string }> = [];
  const lowerQuery = query.toLowerCase();

  for (const [shortcode, emoji] of Object.entries(emojiMap)) {
    if (shortcode.toLowerCase().includes(lowerQuery)) {
      suggestions.push({ shortcode, emoji });
...
export function isEmojiShortcode(
  ...
  return emojiMap.hasOwnProperty(text);
}

export function expandEmojiShortcode(
  shortcode: string,
  emojiMap: EmojiMap = GITHUB_EMOJIS,
): string | null {
